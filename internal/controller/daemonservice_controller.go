							...
							Args: []string{
								fmt.Sprintf(`
                                    while true; do
                                        statuses="";
                                        for service in %s; do
                                            status=$(systemctl is-active $service || echo "not found");
                                            statuses="$statuses{\"serviceName\":\"$service\", \"status\":\"$status\"},"
                                        done;
                                        echo "{\"node\":\"$(hostname)\", \"services\":[$statuses]}";
                                        sleep 10;
                                    done
                                `, strings.Join(daemonService.Spec.ServiceNames, " ")),
							},
